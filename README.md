...
* kernel source code: In the directory "07_Source codes" of [NetDrive](https://download.friendlyelec.com/rk3328), or download from [Github](https://github.com/friendlyarm/kernel-rockchip), the branch name is nanopi4-v4.19.y
...
* pre-built partition image: In the directory "03_Partition image files" of [NetDrive](https://download.friendlyelec.com/rk3328), or download from [server](https://downloads.friendlyelec.com/os-images/rk3328/images)
* compressed root file system tar ball: In the directory "06_File systems" of [NetDrive](https://download.friendlyelec.com/rk3328), or download from [server](https://downloads.friendlyelec.com/rootfs/rk3328)
...
Clone this repository locally, then download and uncompress the [pre-built images](https://downloads.friendlyelec.com/os-images/rk3328/images):
```
...
wget https://downloads.friendlyelec.com/os-images/rk3328/images/friendlycore-focal-arm64-images.tgz
...
*Note: Here we use friendlycore-focal system as an example*  
Clone this repository locally, then download and uncompress the [pre-built images](https://downloads.friendlyelec.com/os-images/rk3328/images), here you need to download the friendlycore-focal and eflasher [pre-built images](https://downloads.friendlyelec.com/os-images/rk3328/images):
```
git clone https://github.com/friendlyarm/sd-fuse_rk3328 -b kernel-4.19 --single-branch sd-fuse_rk3328-kernel4.19
cd sd-fuse_rk3328-kernel4.19
wget https://downloads.friendlyelec.com/os-images/rk3328/images/friendlycore-focal-arm64-images.tgz
tar xvzf friendlycore-focal-arm64-images.tgz
wget https://downloads.friendlyelec.com/os-images/rk3328/images/emmc-flasher-images.tgz
...
```
out/rk3328-eflasher-friendlycore-focal-4.19-arm64-YYYYMMDD.img
```
...
./tools/extract-rootfs-tar.sh rootfs.tar.gz friendlycore-focal-arm64/rootfs
```
or download the filesystem archive from the following URL and extract it:
```
wget https://downloads.friendlyelec.com/rootfs/rk3328/rootfs-friendlycore-focal-arm64.tgz
wget https://downloads.friendlyelec.com/rootfs/rk3328/rootfs-friendlycore-focal-arm64.tgz.sha256
sha256sum -c rootfs-friendlycore-focal-arm64.tgz.sha256
./tools/extract-rootfs-tar.sh rootfs-friendlycore-focal-arm64.tgz
```
Make rootfs to img:
```
...
```
git clone https://github.com/friendlyarm/kernel-rockchip -b nanopi4-v4.19.y --depth 1 kernel
```
...
```
KERNEL_SRC=kernel KCFG=my_defconfig ./build-kernel.sh friendlycore-focal-arm64
```
