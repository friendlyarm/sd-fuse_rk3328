...
这些OS名称是分区镜像文件存放的目录名, 在脚本内亦有严格定义, 所以不能改动, 例如要制作ubuntu-noble-core的SD固件, 可使用如下命令:
```
...
## 获得打包固件所需要的素材
制作固件所需要的素材有:
* 内核源代码: 在[网盘](https://download.friendlyelec.com/rk3328)的 "07_源代码" 目录中, 或者从[此github链接](https://github.com/friendlyarm/kernel-rockchip)下载, 分支为nanopi-r2-v6.6.y
...
## 如何使用
### 重新打包SD卡运行固件
*注: 这里以ubuntu-noble-core系统为例进行说明*  
克隆本仓库到本地, 然后下载并解压ubuntu-noble-core系统的[分区镜像文件压缩包](https://downloads.friendlyelec.com/os-images/rk3328/images):
```
...
wget https://downloads.friendlyelec.com/os-images/rk3328/images/ubuntu-noble-core-arm64-images.tgz
tar xvzf ubuntu-noble-core-arm64-images.tgz
```
解压后, 会得到一个名为ubuntu-noble-core-arm64的目录, 可以根据项目需要, 对目录里的文件进行修改, 例如把rootfs.img替换成自已修改过的文件系统镜像, 或者自已编译的内核和uboot等, 准备就绪后, 输入如下命令将系统映像写入到SD卡  (其中/dev/sdX是你的SD卡设备名):
```
...
*注: 这里以ubuntu-noble-core系统为例进行说明*  
克隆本仓库到本地, 然后下载并解压[分区镜像文件压缩包](https://downloads.friendlyelec.com/os-images/rk3328/images), 这里需要下载ubuntu-noble-core和eflasher系统的文件:
```
git clone https://github.com/friendlyarm/sd-fuse_rk3328 -b kernel-6.6.y --single-branch sd-fuse_rk3328-kernel6.6
cd sd-fuse_rk3328-kernel6.6
wget https://downloads.friendlyelec.com/os-images/rk3328/images/ubuntu-noble-core-arm64-images.tgz
...
或者从以下网址下载文件系统压缩包并解压:
```
wget https://downloads.friendlyelec.com/rootfs/rk3328/rootfs-ubuntu-noble-core-arm64.tgz
wget https://downloads.friendlyelec.com/rootfs/rk3328/rootfs-ubuntu-noble-core-arm64.tgz.sha256
