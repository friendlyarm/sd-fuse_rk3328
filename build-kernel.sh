...
function usage() {
       echo "Usage: $0 <friendlycore-lite-noble-arm64|debian-bookworm-core-arm64|openmediavault-arm64|friendlywrt23|friendlywrt23-docker|friendlywrt21|friendlywrt21-docker|eflasher>"
       echo "# example:"
       ...
       echo "    LOGO=/tmp/logo.bmp KERNEL_LOGO=/tmp/logo_kernel.bmp ./build-kernel.sh friendlycore-lite-noble-arm64"
       echo "    ./mk-emmc-image.sh friendlycore-lite-noble-arm64"
       echo "# also can do:"
       echo "    KERNEL_SRC=~/mykernel ./build-kernel.sh friendlycore-lite-noble-arm64"
       echo "# other options, build kernel-headers, enable/disable 3rd drivers:"
       echo "    MK_HEADERS_DEB=1 BUILD_THIRD_PARTY_DRIVER=0 ./build-kernel.sh friendlycore-lite-noble-arm64"
       exit 0
}
